...
actual abstract class DescopeOAuth {
     ...
     * @return an [AuthenticationResponse] upon successful authentication.
     */
    abstract suspend fun native(context: Context, provider: OAuthProvider, options: List<SignInOptions>? = null): AuthenticationResponse

    /** @see native */
    actual abstract fun native(provider: OAuthProvider, options: List<SignInOptions>?, callback: (Result<AuthenticationResponse>) -> Unit)

    /** @see native */
    abstract fun native(context: Context, provider: OAuthProvider, options: List<SignInOptions>? = null, callback: (Result<AuthenticationResponse>) -> Unit)
}
...
actual abstract class DescopePasskey {
     ...
     */
    @RequiresApi(value = Build.VERSION_CODES.P)
    abstract suspend fun signUp(context: Context, loginId: String, details: SignUpDetails?): AuthenticationResponse
     ...
     */
    @RequiresApi(value = Build.VERSION_CODES.P)
    abstract suspend fun signIn(context: Context, loginId: String, options: List<SignInOptions>?): AuthenticationResponse
     ...
     */
    @RequiresApi(value = Build.VERSION_CODES.P)
    abstract suspend fun signUpOrIn(context: Context, loginId: String, options: List<SignInOptions>?): AuthenticationResponse
     ...
     */
    @RequiresApi(value = Build.VERSION_CODES.P)
    abstract suspend fun add(context: Context, loginId: String, refreshJwt: String)
}
...
actual abstract class DescopeFlowRunner {
    ...
    /** @see start */
    abstract fun start(context: Context, callback: (Result<Unit>) -> Unit)
     ...
     * @param incomingUriString the URI received when redirecting back to the app.
     */
    abstract fun resume(context: Context, incomingUriString: String)
...
actual abstract class DescopeFlowPresentation {
    /**
     * Provide your own [CustomTabsIntent] that will be used when [DescopeFlowRunner.start]
     * or [DescopeFlowRunner.resume] are called. You can configure it to suit
     * your application's presentation needs.
     * @return A [CustomTabsIntent]. Returning `null` will use the default custom tab intent.
     */
    abstract fun createCustomTabsIntent(): CustomTabsIntent?
}