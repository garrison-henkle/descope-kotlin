...
internal data class JwtServerResponse(
    ...
    companion object {
        fun fromJson(json: String, cookies: List<HttpCookie>) = createJsonObject(json).run {
            ...
            cookies.forEach {
                when (it.getName()) {
                    SESSION_COOKIE_NAME -> sessionJwt = it.getValue()
                    REFRESH_COOKIE_NAME -> refreshJwt = it.getValue()
                }
            }
            ...
            JwtServerResponse(
                ...
                firstSeen = optBoolean("firstSeen", false),
            )
        }
    }
}
...
internal data class UserResponse(
    ...
    companion object {
        ...
        fun fromJson(json: String, cookies: List<HttpCookie>) = fromJson(createJsonObject(json))
        ...
        fun fromJson(json: JSONObject) = json.run {
            UserResponse(
                ...
                verifiedEmail = optBoolean("verifiedEmail", false),
                phone = stringOrEmptyAsNull("phone"),
                verifiedPhone = optBoolean("verifiedPhone", false),
    ...
    companion object {
        ...
        fun fromJson(json: String, cookies: List<HttpCookie>) = createJsonObject(json).run {
            PasswordPolicyServerResponse(
                ...
                lowercase = optBoolean("lowercase", false),
                uppercase = optBoolean("uppercase", false),
                number = optBoolean("number", false),
                nonAlphanumeric = optBoolean("nonAlphanumeric", false),
            )
        }
    }
}
    ...
    companion object {
        ...
        fun fromJson(json: String, cookies: List<HttpCookie>) = createJsonObject(json).run {
            TotpServerResponse(
                ...
                image = getString("image").encodeToByteArray(),
                key = getString("key"),
            )
        }
    }
