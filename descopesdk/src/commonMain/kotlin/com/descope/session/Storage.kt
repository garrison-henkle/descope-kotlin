...
class SessionStorage(private val projectId: String, logger: DescopeLogger? = null, store: Store? = null) : DescopeSessionStorage {
    ...
    private val store: Store = store ?: createdEncryptedStore(projectId, logger)
    ...
    private data class Value(
        ...
        companion object {
            fun deserialize(string: String): Value = createJSONObject(string).run {
                Value(
...
private fun deserializeDescopeUser(json: JSONObject): DescopeUser = json.run {
    DescopeUser(
        ...
        picture = stringOrEmptyAsNull("picture")?.run { parseUri(this) },
        email = stringOrEmptyAsNull("email"),
        isVerifiedEmail = optBoolean("isVerifiedEmail", false),
        phone = stringOrEmptyAsNull("phone"),
        isVerifiedPhone = optBoolean("isVerifiedPhone", false),
...
private fun DescopeUser.toJson() = createJSONObject().apply {
    put("userId", userId)
...
internal expect fun createdEncryptedStore(projectId: String, logger: DescopeLogger?): SessionStorage.Store